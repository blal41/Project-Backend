...
const registerUser = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiResponse(200, createuser, "user registeres seccessfully"));
});
...
const getUserChannerProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    {
      $lookup: {
        from: "subscriptions", // when it save then it become a lowercase and add s
        localField: "_id",
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    ...
    {
      $lookup: {
        from: "videos", // when it save then it become a lowercase and add s
        localField: "watchHistory",
        foreignField: "_id",
        as: "watchHistory",
        pipeline: [{
          $lookup: {
            from: "users", // when it save then it become a lowercase and add s
            localField: "owner",
            foreignField: "_id",
            as: "owner",
            pipeline : [{
